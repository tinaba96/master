...
typedef struct {
    ...
    XKerneldl_kerneldl_uio_map maps[ MAX_UIO_MAPS ];
} XKerneldl_kerneldl_uio_info;

/***************** Variable Definitions **************************************/
static XKerneldl_kerneldl_uio_info uio_info;
...
static int uio_info_read_name(XKerneldl_kerneldl_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XKerneldl_kerneldl_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XKerneldl_kerneldl_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XKerneldl_kerneldl_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XKerneldl_kerneldl_Initialize(XKerneldl_kerneldl *InstancePtr, const char* InstanceName) {
	XKerneldl_kerneldl_uio_info *InfoPtr = &uio_info;

		...
		*/
		...
		for(int j = 0; j < 320*320; j++)
		{
			...
			*/
			...
			if(isnan(gradswhf[j]) || isinf(gradswhf[j]))
			{
				//printf("grawxf_error"); 
				//exit(1);
			}
			if(isnan(gradswhg[j]) || isinf(gradswhg[j]))
			{
				//printf("grawxf_error"); 
				//exit(1);
			}
			...
			if(isnan(gradswhi[j]) || isinf(gradswhi[j]))
			{
				//printf("grawxf_error"); 
				//exit(1);
			}
			if(isnan(gradswho[j]) || isinf(gradswho[j]))
			{
				//printf("grawxf_error"); 
				//exit(1);
			}
		...
		for(int k =0; k < 320; k++)
		{
			for(int i = 0; i < 320; i++)
			{
				...
				if(isnan(dout[320*t+i]) || isinf(dout[320*t+i]))
				{
					//printf("grawxf_error"); 
					//exit(1);
				}
